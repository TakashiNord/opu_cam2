cam2

Purpose:
  Программа изменяет значения подач в операциях - не открывая их.

Requirements for this package:
  UG V17/V18/NX1/NX2/....
  WindowsNT/2000/XP
  UG/Gateway
  UG/Open API Execute

Restrictions:
  not

Author:
  ЧЕ


Quick installation:
   Move 'cam2.dll' to one of the following directories:
     %UGII_USER_DIR%\application 
     %UGII_SITE_DIR%\application 
     %UGII_VENDOR_DIR%\application 
  - for calling 'cam2.dll' via File->Execute UG/Open->User Function or User Tool:
      Move 'cam2.dll' to one of the following directories:
        %UGII_USER_DIR%\application 
        %UGII_SITE_DIR%\application 
        %UGII_VENDOR_DIR%\application 

  The required enironment-variable (i.e. UGII_USER_DIR or UGII_SITE_DIR or
  UGII_VENDOR_DIR) must be set properly on your system.

Most common problem:
  none

Copyright & remarks:
  Thanks to my collegs.
  If you have added an interesting enhancement/port or want to report
  an error, please contact the author at mail

History:
  12-May-2006: V1.0.0, Initial release V1.0, UG V16
